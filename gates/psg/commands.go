...
func (p *Psg) RecordsGet(record dto.Record) (result []dto.Record, err error) {
	...
	defer rows.Close()
	fmt.Println(rows) // FOR DEBUGGING
	for rows.Next() {
		var r dto.Record
...
func (p *Psg) RecordUpdate(record dto.Record) error {
	// TODO: Decide is it needed to handle multiple instances from SELECT?
	sqlCommand := `UPDATE address_book SET name=$1, last_name=$2, middle_name=$3, address=$4 WHERE phone=$5`
...
`
	...
	res_query = sb.String()
	fmt.Println(res_query) // FOR DEBUGGING
	fmt.Println(values)
	return
}
